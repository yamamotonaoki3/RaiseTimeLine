...
RaiseTimeLine backend に対する負荷テスト・ストレステスト・スパイクテストを [k6](https://k6.io/) で実行するための一式です。
...
すべてのシナリオは `TEST_TYPE` 環境変数で **負荷テスト（load）／ストレステスト（stress）／スパイクテスト（spike）** を切り替えられます。

| テスト種別 | 目的 | 負荷パターン |
| --- | --- | --- |
| `load` | 通常運用の負荷でSLA（p95<1000ms, エラー率<1%）を満たすか確認 | 10 VU を5分間維持 |
| `stress` | システムの限界点・劣化ポイントを見つける | 10→50→100 VU と段階的に増加 |
| `spike` | 急なアクセス集中への耐性確認 | 10 VU→100 VU（急増）→10 VU（急減） |
...
いずれかを満たさない場合、k6はexit codeが非0で終了します。`load`テストで閾値を満たさない場合は性能上の問題があるとみなし、`stress`/`spike`テストの結果は「どのVU数で劣化し始めるか」を確認する参考情報として扱います。
...
このディレクトリは `.gitignore` で除外されており、レポートファイル自体はコミットされません。
...
├── lib/
│   ├── auth.js              # ログイン共通処理
│   ├── scenarios-config.js  # load/stress/spike の設定
