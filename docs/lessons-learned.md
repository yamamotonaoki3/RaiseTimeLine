...
- 2026-08-11: トランザクション境界のない処理で「記録→作成」の順にしたため、その隙間に届いたリクエストが401になった件
- 2026-08-11: E2Eが広範囲に失敗したとき、mainと比較して自分の変更が原因だと確定させた件
...
- **種別**: 不具合修正（実装中にE2Eの大量失敗で発覚した設計ミス）
- **対象領域・関連ファイル**: backend/src/main/java/com/raisetimeline/api/auth/refreshtoken/RefreshTokenService.java（rotate / handleAlreadyUsed）, backend/src/main/java/com/raisetimeline/api/auth/AuthService.java（refreshSession）
- **何が起きたか**: #71/#72 で、リフレッシュトークンのローテーションを「削除」から「使用済みの記録」に変えた。最初の実装は次の順序だった。
  1. 古いトークンを使用済みにし、`replaced_by` に新しいトークンの値を記録する（UPDATE）
  2. 新しいトークンの行を作る（INSERT）

  `AuthService.refreshSession` は `@Transactional` を持たず、各SQLが即コミットされる。そのため**1と2の間に、`replaced_by` は記録済みなのに参照先の行がまだ存在しない状態**が生まれる。この隙間に届いた別のリクエストは「連鎖が途切れている＝無効」と判断して401を返していた。

  症状はE2Eに出た。41件中6件が失敗し、所要時間も**42秒から3分に悪化**。失敗するテストは実行ごとに変わり、画像の読み込み失敗まで起きたため、当初は環境負荷を疑った。
- **切り分け**: `git stash` で変更を退避し、**main のコードで同じE2Eを実行**したところ40件が42.6秒で全通過した。これで自分の変更が原因だと確定できた。その後、バックエンドのアクセスログで `/api/auth/refresh` の401が**14件（main）→47件（変更後）**に増えていることを確認し、原因の箇所を特定した。
- **対応**: 順序を逆にして、**先に新しい行を作ってから使用済みにする**。参照先が常に存在するため隙間が消える。競合で使用済みにできなかった場合は、自分が作った行を削除して相手の発行したトークンを共有する。
- **もう1つの手直し**: 「置き換え先が使用済みなら盗用とみなす」という判定は厳しすぎた。短時間に何度も画面遷移すると `T1→T2→T3` と進んだ後に T1 を積んだ遅れたリクエストが正常に届く。**連鎖をたどって先端を返す**方式に改め、正常利用での誤検知が28件から0件になった。
- **次回の行動規則**:
  1. **参照関係のあるデータを複数のSQLで書くときは、「参照される側」を先に作る。** トランザクション境界がなければ、途中の状態が他のリクエストから見える。「記録してから作る」順序は、その隙間に必ず窓を開ける。トランザクションで囲むより、順序で隙間そのものを無くすほうが単純で確実。
  2. **並行アクセスを扱う機能では、「正常な競合」と「異常」を分ける条件を実データで確かめる。** 机上で単純化した判定（今回は「置き換え先が使用済みなら盗用」）は、実際の利用パターンでは厳しすぎることがある。計画時に単純化した箇所は、実装後に必ず実測で妥当性を確認する。
  3. **認証・セッションまわりを変更したら、変更後のエラー応答の件数をログで数える。** 今回は401の件数比較が決め手になった。「テストが落ちる」だけでは原因の場所が分からない。
- **状態**: 有効
- **根拠**: Issue #71 / #72 / PR #86

### 2026-08-11: E2Eが広範囲に失敗したとき、mainと比較して自分の変更が原因だと確定させた件

- **種別**: 調査手法（今後も再利用できる切り分け手順）
- **対象領域・関連ファイル**: frontend/e2e/, backend/（バックエンドの変更がE2Eに波及したケース）
- **何が起きたか**: E2Eが41件中6件失敗し、**失敗するテストが実行ごとに変わった**。所要時間も42秒→3分に悪化し、画像の読み込み失敗まで起きた。この症状は「マシンの負荷」「Dockerの不調」「開発サーバの劣化」のように環境要因に見え、実際そちらを先に疑って残留プロセスの掃除などに時間を使った。しかし原因は自分のバックエンド変更だった。
- **決め手になった手順**:
  1. `git stash push -u` で変更を退避する
  2. その状態（＝main相当）でバックエンドを再ビルドし、**同じ手順でE2Eを実行**する
  3. 全通過するなら自分の変更が原因、同じように失敗するなら環境要因
  4. `git stash pop` で戻す
- **次回の行動規則**:
  1. **「失敗するテストが毎回変わる」「全体が遅くなる」を環境要因と決めつけない。** 並行処理・認証まわりの変更は、この形で症状が出る。個々のテストの失敗内容を追うより、**mainとの比較を先に1回やる**ほうが早い。
  2. **比較は「同じ手順・同じ順序」で行う。** 単体のspecだけ再実行して通っても切り分けにならない（今回、単独実行では通ってしまい判断を誤りかけた）。通し実行で比較する。
  3. **アプリのアクセスログを比較材料に使う。** ステータスコードの分布（例: 401の件数）を main と変更後で比べると、どの経路が壊れたかが一目で分かる。
- **状態**: 有効
- **根拠**: Issue #71 / #72 / PR #86
