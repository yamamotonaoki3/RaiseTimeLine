...
export class HomePage {
   ...
   * 「全体」タブに切り替える。
   *
   * リロード直後は前のDOMが残っていることがあり、単に allTab.click() すると
   * クリックが空振りすることがある。画面の描画と初回読み込みの完了を待ってから切り替える。
   *
   * （フィード切替時に古いレスポンスで表示が上書きされる不具合は #69 で修正済み。
   * ここでの待機は安定性のためのものであり、不具合の回避策ではない）
   */
  async showAllFeed() {
